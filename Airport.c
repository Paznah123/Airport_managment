...
void	printAirport(const Airport* pPort)
{
	...
	printf("Country: %-20s\t", pPort->country);
	printf("Code:%s\n",pPort->code);
...
void	getAirportContry2(Airport* pPort)
{
	char temp[255];
	char** wordsArray = NULL;
	int totalLength;
	...
	wordsArray = splitCharsToWords(temp, &count, &totalLength);
	pPort->country = (char*)calloc(totalLength + (count - 1) * 2 + 1, sizeof(char));
	if (!pPort->country)
	{
		for (int i = 0; i < count; i++)
			free(wordsArray[i]);
